...
- `src/store/auth.ts` keeps JWT + reviewer profile in Zustand (including the lowercase username returned by the API), persists them to `localStorage`, and injects the token into the shared Axios client (`src/lib/api-client.ts`).  
- `src/pages/RegisterPage.tsx` handles account creation against `/auth/register`, captures name/email/username/password, and automatically signs reviewers in once the API provisions the default wallet.  
